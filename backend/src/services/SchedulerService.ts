...
export class SchedulerService {
   ...
   */
  static async getDueOrganizations(): Promise<Array<{ id: string; name: string; apiKey: string; region: string; config: ScheduleConfig }>> {
    const result = await query(
      `SELECT id, meraki_org_name, meraki_api_key_encrypted AS meraki_api_key, meraki_region, schedule_config
       FROM organizations
