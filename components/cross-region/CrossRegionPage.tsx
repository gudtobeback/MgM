...
import { Globe2, ArrowLeftRight, Network, Cpu, Server, Wifi, Loader2, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold text-foreground tracking-tight">Cross-Region Sync</h1>
        </div>
      ...
      <div className="glass-card p-6">
        <h2 className="font-semibold text-foreground mb-4">Select Organizations to Compare</h2>
        {loadingOrgs ? (
      ...
      {report && (
        <>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {[
              { label: 'Source', org: report.sourceOrg, accent: 'from-blue-500 to-cyan-500' },
              { label: 'Target', org: report.targetOrg, accent: 'from-purple-500 to-violet-500' },
            ].map(({ label, org, accent }) => (
              <div key={label} className="glass-card p-4 flex items-center gap-3">
                <div
                  className="w-10 h-10 rounded-xl flex items-center justify-center text-white shrink-0"
                  style={{ background: `linear-gradient(135deg, ${label === 'Source' ? '#3b82f6, #06b6d4' : '#a855f7, #7c3aed'})` }}
                >
                  <Globe2 size={16} />
                </div>
                <div>
          ...
          <div className="glass-card p-5">
            <h2 className="font-semibold text-foreground mb-4">Comparison Summary</h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {(['networks', 'devices', 'vlans', 'ssids'] as const).map(cat => {
                  ...
                  return (
                    ...
                    >
                      <div className={`flex justify-center mb-1.5 ${isActive ? 'text-blue-500' : 'text-muted-foreground'}`}>
                        {CATEGORY_ICONS[cat]}
                      </div>
                      <div className="text-2xl font-bold text-blue-600">{catDiffs.length}</div>
                      <div className="text-xs text-muted-foreground capitalize mt-0.5">{cat} differences</div>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="glass-card overflow-hidden">
              <div className="flex items-center justify-between p-4 border-b border-white/40">
                <h2 className="font-semibold text-foreground">
                  ...
                  <span className="ml-2 text-sm font-normal text-muted-foreground">({filteredDiffs.length})</span>
                </h2>
                {categoryFilter !== 'all' && (
                  <button
                  ...
                  <div key={i}>
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <span className="text-muted-foreground shrink-0">{CATEGORY_ICONS[diff.category]}</span>
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-foreground truncate">{diff.item}</p>
                          <span className="text-xs capitalize text-muted-foreground">{diff.category}</span>
                        </div>
                      </div>
