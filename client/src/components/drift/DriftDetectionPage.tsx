...
export const DriftDetectionPage: React.FC<DriftDetectionPageProps> = ({
  ...
  useEffect(() => {
    ...
    apiClient
      .listSnapshots(organizationId, { limit: 50 })
      .then(setSnapshots)
      .catch(() => {})
  ...
  const runDriftCheck = async () => {
    ...
    setError("");
    try {
      const data = await apiClient.detectDrift(
        organizationId,
        goldenId || undefined,
      );
      setReport(data);
      ...
      setError(
        err.message ||
          "Failed to run drift detection. Make sure you have at least 2 snapshots.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredDrifts =
    report?.drifts.filter((d) => filter === "all" || d.severity === filter) ??
    [];

  const severityCounts = {
    critical:
      report?.drifts.filter((d) => d.severity === "critical").length ?? 0,
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {organizationName ? `${organizationName} — ` : ""}Compares the
            current config against a pinned golden config snapshot.
          </p>
        </div>
          ...
          {report && (
            <button
              onClick={() =>
                downloadJSON(
                  report,
                  `drift-report-${organizationId}-${new Date().toISOString().slice(0, 10)}.json`,
                )
              }
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-foreground">
                Golden Config (Baseline)
              </p>
              {goldenSnapshot ? (
                <p className="text-xs text-muted-foreground mt-0.5 truncate">
                  <span className="font-medium text-amber-700">
                    {TYPE_LABEL[goldenSnapshot.snapshotType] ??
                      goldenSnapshot.snapshotType}
                  </span>
                  {" — "}
                  <CalendarClock size={10} className="inline -mt-0.5 mr-0.5" />
                  {fmtDate(goldenSnapshot.createdAt)}
                  {goldenSnapshot.notes && (
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  No golden config selected. Comparing the two most recent
                  snapshots (not true drift detection).
                </p>
              )}
            </div>
          </div>
          ...
          <div className="mt-4 border-t border-white/30 pt-4">
            ...
            {loadingSnaps ? (
              ...
              <p className="text-sm text-muted-foreground py-2">
                No snapshots found. Create one from Version Control first.
              </p>
            ) : (
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
          <Loader2
            size={40}
            className="mb-4 text-blue-500 opacity-50 animate-spin"
          />
          <p className="text-sm">
            Comparing snapshots for configuration drift…
          </p>
        </div>
      )}
      ...
      {!loading && report && (
        <>
          ...
          <div className="glass-card px-4 py-3 grid grid-cols-2 gap-4 text-xs text-muted-foreground">
            <div>
              <p className="font-semibold text-foreground mb-0.5 flex items-center gap-1.5">
                <Star size={11} className="text-amber-500" /> Golden Config
                (Baseline)
              </p>
              <p>{fmtDate(report.baselineCreatedAt)}</p>
              <p className="font-mono opacity-60 mt-0.5">
                {report.baselineSnapshotId.slice(0, 8)}… ·{" "}
                {TYPE_LABEL[report.baselineSnapshotType] ??
                  report.baselineSnapshotType}
              </p>
            </div>
            <div>
              <p className="font-semibold text-foreground mb-0.5 flex items-center gap-1.5">
                <CalendarClock size={11} className="text-blue-500" /> Current
                (Latest Snapshot)
              </p>
              <p>{fmtDate(report.currentCreatedAt)}</p>
              <p className="font-mono opacity-60 mt-0.5">
                {report.currentSnapshotId.slice(0, 8)}…
              </p>
            </div>
          </div>
              ...
              {report.totalDrifts === 0 && (
                <div className="text-center py-14 border border-dashed border-green-200/60 bg-green-50/40 rounded-xl backdrop-blur-sm">
                  ...
                  <p className="text-sm text-green-700 mt-1">
                    Configuration matches the golden config. No unauthorized
                    changes found.
                  </p>
                </div>
              )}
              ...
              {filteredDrifts.length > 0 && (
                <div className="space-y-2">
                  {filteredDrifts.map((drift, idx) => {
                    const hasValues =
                      drift.oldValue !== undefined ||
                      drift.newValue !== undefined;
                    const isExpanded = expandedDrifts.has(idx);
                    const ResourceIcon =
                      RESOURCE_ICON[drift.resourceType] ?? Settings;
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          SEVERITY_BORDER[drift.severity] ??
                            "border-l-gray-300",
                        )}
                      >
                        {/* Row header */}
                              ...
                              <div className="flex items-center gap-2 mt-1 flex-wrap">
                                ...
                                {drift.field && (
                                  <span className="text-xs text-muted-foreground">
                                    field:{" "}
                                    <code className="bg-white/50 border border-white/40 px-1 rounded text-[10px] font-mono">
                                      {drift.field}
                                    </code>
                                  </span>
                                )}
          ...
          <p className="text-xs text-center text-muted-foreground">
            Checked: {fmtDate(report.checkedAt)} &nbsp;·&nbsp;{" "}
            {report.totalDrifts} change{report.totalDrifts !== 1 ? "s" : ""}{" "}
            found
          </p>
        </>
      )}
      ...
      {!loading && !report && !error && (
        <div className="text-center py-16 border border-dashed border-white/40 bg-white/20 rounded-xl backdrop-blur-sm">
          <GitCompare
            size={44}
            className="mx-auto mb-3 text-blue-400 opacity-70"
          />
          <h3 className="font-semibold text-foreground text-lg">
            Ready to Scan
          </h3>
          <p className="text-sm text-muted-foreground mt-1">
            Pin a golden config above, then click "Run Check" to detect
