  ...
  switch (changeType) {
    ...
    case "access-policy":
      return {
        name: "Dot1x Policy",
        radiusServers: [
          { host: "1.2.3.4", port: 1812, secret: "radius_secret" },
        ],
...
export const PushChangePanel: React.FC<PushChangePanelProps> = ({
  change,
  onComplete,
  onCancel,
}) => {
  ...
  const [selectedOrg, setSelectedOrg] = useState<MerakiOrganization | null>(
    null,
  );
  const [networks, setNetworks] = useState<MerakiNetwork[]>([]);
  const [selectedNetwork, setSelectedNetwork] = useState<MerakiNetwork | null>(
    null,
  );
  ...
  const [deviceSerial, setDeviceSerial] = useState(
    change.affected_resources?.devices[0] ?? "",
  );
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      setConnectError(
        e.message || "Failed to connect. Check your API key and region.",
      );
    } finally {
      setConnecting(false);
    }
  ...
  const canGoToPayload =
    !!selectedNetwork &&
    (change.change_type === "firmware" || change.change_type !== "firmware");
  ...
  const handlePush = async () => {
    ...
    try {
      ...
      setPayloadError(
        "Invalid JSON — please fix the syntax above before pushing.",
      );
      return;
    }
    ...
    try {
      addLog(
        `── Pushing "${change.title}" to network "${selectedNetwork.name}" ──`,
      );
      ...
      if (ct === "vlan") {
        ...
        await updateNetworkApplianceFirewallL3FirewallRules(
          apiKey,
          region,
          networkId,
          parsed,
        ...
        if (!deviceSerial || !portId)
          throw new Error(
            "Device serial and port ID are required for switch-port changes.",
          );
        ...
        if (!deviceSerial)
          throw new Error("Device serial is required for device changes.");
        ...
        addLog(
          "Please use the Meraki Dashboard to schedule firmware upgrades.",
        );
      } else {
    ...
    >
      ...
      <div style={{ padding: "18px 20px" }}>
        ...
        {change.change_type === "firmware" && (
          ...
          >
            Firmware upgrades cannot be triggered via the Meraki API from this
            tool. Please schedule the upgrade directly in the Meraki Dashboard
            under Network → Firmware upgrades.
          </div>
        )}
        ...
        {(phase === "connect" || phase === "payload") && (
          ...
          >
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: "13px",
                  fontWeight: 600,
                  cursor: !apiKey || connecting ? "not-allowed" : "pointer",
                  backgroundColor:
                    !apiKey || connecting ? "#6b7280" : "#2563eb",
                  color: "#fff",
                  border: "none",
                  display: "flex",
                  alignItems: "center",
            ...
            {(change.change_type === "switch-port" ||
              change.change_type === "device") && (
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns:
                    change.change_type === "switch-port" ? "1fr 1fr" : "1fr",
                  gap: "10px",
                }}
              >
                <div>
            ...
            {change.change_type === "ssid" && (
              <div style={{ maxWidth: "180px" }}>
                ...
                <input
                  ...
                  max={15}
                  value={ssidNumber}
                  onChange={(e) => setSsidNumber(e.target.value)}
                />
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: "8px", paddingTop: "4px" }}>
              {phase === "connect" && (
                <button
                  ...
                  disabled={
                    !canGoToPayload && change.change_type !== "firmware"
                  }
                  style={{
                    padding: "7px 16px",
                    borderRadius: "4px",
        ...
        {(phase === "pushing" || phase === "done") && (
          <div>
            <div style={SECTION}>
              {phase === "pushing" ? "Pushing change…" : "Push result"}
            </div>
            <div
              ref={logRef}
              style={{
