...
class ApiClient {
  ...
  async updateAdminUser(
    id: number,
    data: { role?: string; companyId?: number; subscriptionTier?: string },
  ) {
  ...
  async updateUserPermissions(
    ...
    const response = await this.request(
      `/company/users/${userId}/permissions`,
      {
        method: "PATCH",
  ...
  async deleteCompanyUser(userId: number) {
    const response = await this.request(`/company/users/${userId}`, {
      method: "DELETE",
    });
    if (!response.ok) {
      const err = await response.json();
  ...
  async removeOrganization(orgId: string): Promise<void> {
    const response = await this.request(`/organizations/${orgId}`, {
      method: "DELETE",
    });
    if (!response.ok) {
      const error = await response.json();
  ...
  async refreshOrganization(orgId: string) {
    const response = await this.request(`/organizations/${orgId}/refresh`, {
      method: "POST",
    });
   ...
   */
  async getOrganizationCredentials(
    orgId: string,
  ): Promise<{ api_key: string; region: string }> {
  ...
  async detectDrift(organizationId: string, baselineSnapshotId?: string) {
    const params = baselineSnapshotId
      ? `?baselineSnapshotId=${encodeURIComponent(baselineSnapshotId)}`
      : "";
    const response = await this.request(
      `/organizations/${organizationId}/drift${params}`,
    );
    if (!response.ok) {
      const error = await response.json();
  ...
  async runComplianceCheck(organizationId: string, snapshotId?: string) {
    ...
    const response = await this.request(
      `/organizations/${organizationId}/compliance${params}`,
    );
    if (!response.ok) {
      const error = await response.json();
  ...
  async getBulkNetworks(organizationId: string) {
    const response = await this.request(
      `/organizations/${organizationId}/bulk/networks`,
    );
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || "Failed to fetch networks");
  ...
  async bulkApplyVlan(
    ...
    const response = await this.request(
      `/organizations/${organizationId}/bulk/vlans`,
      {
        method: "POST",
  ...
  async bulkApplyFirewall(
    organizationId: string,
    networkIds: string[],
    rules: any[],
  ) {
    const response = await this.request(
      `/organizations/${organizationId}/bulk/firewall`,
      {
        method: "POST",
  ...
  async bulkApplyTags(
    ...
    const response = await this.request(
      `/organizations/${organizationId}/bulk/tags`,
      {
        method: "POST",
  ...
  async createSnapshot(
    ...
    const response = await this.request(
      `/organizations/${organizationId}/snapshots`,
      {
        method: "POST",
  ...
  async getSnapshot(
    ...
    const response = await this.request(
      `/organizations/${organizationId}/snapshots/${snapshotId}`,
    );
  ...
  async getSecurityPosture(organizationId: string, snapshotId?: string) {
    const params = snapshotId ? `?snapshotId=${snapshotId}` : "";
    const response = await this.request(
      `/organizations/${organizationId}/security${params}`,
    );
    if (!response.ok) {
      const error = await response.json();
  ...
  async listChanges(organizationId: string, status?: string) {
    ...
    const response = await this.request(
      `/organizations/${organizationId}/changes${params}`,
    );
    if (!response.ok) {
      const error = await response.json();
  ...
  async createChangeRequest(
    ...
    const response = await this.request(
      `/organizations/${organizationId}/changes`,
      {
        method: "POST",
        body: JSON.stringify(data),
      },
  ...
  async updateChangeRequest(
    ...
    const response = await this.request(
      `/organizations/${organizationId}/changes/${changeId}`,
      {
        method: "PATCH",
  ...
  async generateDocumentation(organizationId: string, snapshotId?: string) {
    const params = snapshotId ? `?snapshotId=${snapshotId}` : "";
    const response = await this.request(
      `/organizations/${organizationId}/docs${params}`,
    );
    if (!response.ok) {
      const error = await response.json();
  ...
  async getSchedule(organizationId: string) {
    const response = await this.request(
      `/organizations/${organizationId}/schedule`,
    );
    if (!response.ok) {
      const error = await response.json();
  ...
  async triggerScheduledSnapshot(organizationId: string) {
    const response = await this.request(
      `/organizations/${organizationId}/schedule/trigger`,
      { method: "POST" },
    );
    if (!response.ok) {
      const error = await response.json();
  ...
  async compareOrgs(sourceOrgId: string, targetOrgId: string) {
    ...
    const response = await this.request(
      `/cross-region/compare?${params.toString()}`,
    );
    if (!response.ok) {
      const error = await response.json();
