...
app.post("/api/proxy", async (req, res) => {
  try {
    const { apiKey, region, endpoint, method, body } = req.body as {
      apiKey: string;
      region: string;
      endpoint: string;
      method: string;
    ...
    if (!apiKey || !region || !endpoint || !method) {
      return res
        .status(400)
        .json({ error: "Missing required parameters in proxy request" });
    }
    ...
    res.setHeader(
      "Content-Type",
      merakiResponse.headers.get("Content-Type") || "application/json",
    );
    ...
    res
      .status(500)
      .json({ error: `An internal server error occurred: ${errorMessage}` });
  }
});
  ...
  try {
    ...
    for (const org of due) {
      try {
        ...
        await SchedulerService.pruneOldSnapshots(
          org.id,
          org.config.retainCount,
        );
